...
% ── Additional packages (kept minimal, as LIPIcs requires) ────────────────────
\usepackage{booktabs}
\usepackage{tabularx}
\usepackage{pifont}     % check / cross marks in the verification table
\usepackage{tikz}
\usetikzlibrary{arrows.meta, positioning}

% ── Colours (xcolor is provided by the LIPIcs class) ──────────────────────────
...
% ── Float placement tuning (reduce whitespace around figures) ─────────────────
% Allow figures to fill more of a page and top-align any float-only page so it
% is not vertically centred (which left large gaps above/below figures).
...
% ── Metadata (mandatory LIPIcs macros) ────────────────────────────────────────
\title{Generating and Solving 3D Tetracube Puzzles in Arbitrary
       Non-Rectangular Volumes with ASP}
\titlerunning{Non-Rectangular Tetracube Puzzles with ASP}

\author{Abdelhadi Agourzam}{Universit\'e de Mons (UMONS), Facult\'e des
  Sciences, Belgium}{abdelhadiagourzam@gmail.com}{}{}
\author{Mohamed El ismaily}{Universit\'e de Mons (UMONS), Facult\'e des
  Sciences, Belgium}{}{}{}

\authorrunning{A. Agourzam and M. El ismaily}
\Copyright{Abdelhadi Agourzam and Mohamed El ismaily}

\ccsdesc[500]{Computing methodologies~Knowledge representation and reasoning}
\ccsdesc[300]{Theory of computation~Constraint and logic programming}

\keywords{Answer Set Programming, Tetracubes, 3D Puzzle, Non-Rectangular
  Volumes, Shape-Agnostic, Scalable, Clingo, Exact Cover}

\supplement{Source code: \url{https://github.com/oscarRickovic/KRR2026}}

\acknowledgements{We thank Jef Wijsen for supervising this project.}

% ── Event metadata (course context) ───────────────────────────────────────────
\EventEditors{}
\EventNoEds{0}
\EventLongTitle{Knowledge Representation and Reasoning (KRR 2026)}
\EventShortTitle{KRR 2026}
\EventAcronym{KRR}
\EventYear{2026}
\EventDate{2026}
\EventLocation{Universit\'e de Mons (UMONS), Belgium}
\EventLogo{}
\SeriesVolume{1}
\ArticleNo{1}
...
\begin{abstract}
We design and implement a \emph{shape-agnostic, scalable} system for generating
and solving 3D tetracube puzzles using Answer Set Programming (ASP).  The 8
standard tetracubes (I, T, L, Pyramid, O, N, Z, Z-mirror) are packed into
\emph{any} connected 3D volume by simply declaring its cells --- no
shape-specific solver code is needed.  A single integer constant
\texttt{num\_copies}\,$=N$ scales the puzzle to $8N$ pieces filling $32N$ cells,
from the classical 32-cell problem ($N=1$) up to a 224-cell configuration
($N=7$).  We demonstrate solutions for three non-rectangular shapes (T-shape,
L-shape, Plus/Cross) and randomly generated polycubes at multiple scales, with
empirical solve times ranging from 30\,ms ($N=1$) to 6\,s ($N=7$) on the
slab family.  We further show that, without the slab structure, freely-grown 3D
volumes are almost never packable, evidencing a genuinely two-sided,
conjectured-NP-hard general problem.  An interactive 3D visualiser, an
independent 6-check verifier, and a random puzzle generator complete the system.
...
\begin{enumerate}[\bfseries C1.]
  ...
  \item \textbf{Empirical complexity study.}  Solve times measured for
        $N=1$ to $N=7$ reveal a polynomial growth law $\sim N^{2.5}$ on the
        slab family, contrasted with the hard general case.
\end{enumerate}
...
\begin{tabular}{@{}ll@{}}
  ...
  \texttt{benchmark\_3d.py}   & Timing for non-decomposable free 3D polycubes \\
  \bottomrule
\end{tabular}
\end{table}
...
This is the canonical instance: the 8 distinct tetracubes are placed exactly
once to fill all 32 cells.  Note in particular how the Pyramid --- the only
genuinely 3D piece --- must interlock across both $z$-layers with the surrounding
planar pieces, which is what makes even this smallest case non-trivial to pack
by hand.  Figure~\ref{fig:sol_t} shows the result returned by Clingo.
...
The L-shape replaces the T's symmetric stem with a staircase profile (the
top-right $2\times2$ corner is removed).  Because the boundary is no longer
symmetric, the long pieces (I, N, Z) can no longer be placed as in the T-shape
and must take different orientations to follow the step.  No change to the
encoding is required --- only the \texttt{cell/3} facts differ ---
yet the same engine still finds a valid packing (Figure~\ref{fig:sol_l}).
...
The Plus/Cross is the most geometrically constrained of the three volumes.  Its
four protruding arms are only two cells wide, so any piece covering an arm has
very few legal placements: it must lie flat along the arm and connect back to
the central body without spilling outside the boundary.  These narrow
corner-less regions are what make the shape the hardest of the three to fill,
yet a complete packing still exists (Figure~\ref{fig:sol_cross}).
...
Setting \texttt{num\_copies}\,$=2$ doubles the volume to depth 4 (64 cells) and
the piece set to 16 --- two copies of each tetracube type.  This is the first
multi-copy instance, and it exercises the cyclic type-assignment mechanism of
Section~4.3 without any change to the solver.  As discussed later in
Section~7.4, this stacked-slab volume is in fact decomposable; here we simply
note that the engine returns a correct 16-piece packing
(Figure~\ref{fig:sol_t2}).
...
The three shapes above are hand-designed, so one could suspect they were chosen
to suit the solver.  \emph{Randomly generated} volumes remove that suspicion and
are therefore the decisive test of shape-agnosticism: the engine is handed
shapes that neither author has ever seen, with no hand-tuning of the encoding.
The \texttt{random\_puzzle.py} script supplies them and confirms that the
\emph{same} unmodified \texttt{puzzle.lp} solves them.

\paragraph*{Generation (Eden model).}
A volume is grown one cell at a time: starting from a single seed cell, we
repeatedly pick a uniformly random empty face-neighbour of the current cluster
and add it.  This is the Eden growth model, and it produces connected but
otherwise unbiased, irregular polycubes.  Two modes are provided:
\begin{itemize}
  \item \textbf{Extruded (\texttt{--mode 2d}, default).}  Grow a random 16-cell
        2D polyomino, then extrude it to depth $2N$.  The footprint is arbitrary
        but every layer is identical, so the volume is a \emph{slab}; combined
        with \texttt{--copies N} this gives $32N$ cells for $8N$ pieces.  These
        instances are reliably solvable (see Section~7.4 for why slabs always
        admit a packing once the base does).
  \item \textbf{Free 3D (\texttt{--mode 3d}).}  Grow a connected $32N$-cell
        polycube directly in all three axes.  Layers differ, so no extrusion or
        stacking structure exists --- the hardest, most general case
        (analysed empirically in Section~7.5).
\end{itemize}
The generator retries with fresh seeds until it finds a solvable instance, so a
single command produces a complete random puzzle (shape, hints, and solution)
for any $N$.  Figure~\ref{fig:random} shows three solved extruded instances.
...
Random shapes thus play a double role in this work.  In the \emph{extruded}
mode they are the cleanest demonstration that the engine is truly
shape-agnostic --- arbitrary, unseen footprints are packed with no code change,
and at any scale $N$.  In the \emph{free 3D} mode they become the experimental
probe for the general problem: as Section~7.5 shows, such volumes are almost
never packable, which is precisely what reveals the genuine difficulty of
tetracube packing once the convenient slab structure is removed.
...
A rough upper bound on the number of joint placements is:
\[
  \text{Search space} \;\lesssim\;
  \underbrace{\bigl(32N \times 24\bigr)^{8N}}_{\text{(positions} \times \text{orientations) per piece, over all } 8N \text{ pieces}}
  \;\div\;
  \underbrace{(N!)^{8}}_{\text{interchangeable copies}}
\]

The factors are read as follows.  Each of the $8N$ pieces is placed by choosing
an anchor cell (at most $32N$ choices, the cell count) and an orientation.  A
single piece has at most $24$ orientations --- the maximum over the eight types,
attained by the fully asymmetric L (Table~\ref{tab:pieces}); the other types
have fewer ($3$ to $12$).  Hence each piece has at most $32N \times 24$
placements, and an independent choice for all $8N$ pieces gives the
$(32N \times 24)^{8N}$ numerator.  Note that the constant $86$ used elsewhere is
the \emph{sum} of orientations over the eight \emph{types}
($3\!+\!12\!+\!24\!+\!8\!+\!3\!+\!12\!+\!12\!+\!12$), not the number of
orientations of any one piece, so $24$ --- not $86$ --- is the correct per-piece
factor here.  The denominator $(N!)^{8}$ divides out the symmetry among the $N$
interchangeable copies of each of the $8$ types.

This is only an upper bound on the unpruned space.  In practice Clingo's unit
propagation eliminates the vast majority of it: placing one piece immediately
constrains where the others can go, often reducing the effective branching
factor to near $1$ per decision.

The quantity the grounder actually materialises is much smaller.  The choice
rule generates one ground \texttt{position/5} atom per (piece, orientation,
anchor) triple, i.e.\ $\sum_{\text{pieces}} (32N \times r_{\text{type}})$.
Summed over the $N$ copies of all eight types this is exactly
$32N \times 86 \times N = 2752\,N^{2}$ atoms (verified by grounding with Clingo):

\begin{table}[htbp]
\centering
\caption{Exact ASP grounding size as a function of $N$: number of ground
         \texttt{position/5} atoms, $2752\,N^{2}$ (confirmed by grounding).}
\label{tab:ground}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{@{}rrrr@{}}
  \toprule
  $N$ & Cells ($32N$) & Pieces ($8N$) & Ground \texttt{position/5} atoms ($2752\,N^{2}$) \\
  \midrule
  1 &  32 &  8 &     2\,752 \\
  2 &  64 & 16 &    11\,008 \\
  3 &  96 & 24 &    24\,768 \\
